...
type SummaryType = "events" | "income" | "expenses" | "bills" | null;
...
export function DashboardSummaryModal({
  ...
  return (
    <Modal title={title} open={open} onClose={onClose}>
          ...
          {events.map((event) => (
            <article key={event.id}>
              ...
              <div>
                ...
                <span>{event.date} • {event.start} - {event.end} • {event.memberName}</span>
              </div>
            </article>
          ))}
        </div>
            ...
            {list.map((item) => (
              <article key={item.id}>
                ...
                <div>
                  ...
                  <span>{item.category} • {item.memberName} • {formatCurrency(item.value)}</span>
                </div>
              </article>
            ))}
          </div>
      ...
      {type === "bills" && (
        <div className="summary-list">
          {list.length === 0 && <p className="empty-list">Nenhuma conta pendente.</p>}
          {list.map((item) => (
            <article key={item.id}>
              <CreditCard size={18} />
              <div>
                <strong>{item.description}</strong>
                <span>Vence em {item.dueDate} • {formatCurrency(item.value)}</span>
              </div>
            </article>
          ))}
        </div>
