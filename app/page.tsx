...
export default function MembrosPage() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    if (form.phone.replace(/\D/g, "").length < 12) return setMessage("Informe o telefone com DDI e DDD.");
    ...
    fetch("/api/whatsapp", {
      ...
      body: JSON.stringify({
        phone: newUser.phone,
        message: `✅ Senhor Melo\n\nOlá, ${newUser.name}! Seu perfil foi cadastrado para receber lembretes.`,
      }),
    }).catch(() => {});
      ...
      <section className="auth-layout">
        <form className="auth-card" onSubmit={handleSubmit}>
          ...
          <label className="field-with-icon">
            <User size={18} />
            <input placeholder="Nome completo" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} autoComplete="name" />
          </label>

          <label className="field-with-icon">
            <Mail size={18} />
            <input placeholder="E-mail" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} autoComplete="email" />
          </label>

          <label className="field-with-icon">
            <Phone size={18} />
            <input placeholder="Telefone WhatsApp com DDI" value={form.phone} onChange={(event) => setForm({ ...form, phone: maskPhone(event.target.value) })} autoComplete="tel" />
          </label>
          ...
          <div className="password-row">
            <input placeholder="Senha" type={showPassword ? "text" : "password"} value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} />
            <button type="button" onClick={() => setShowPassword(!showPassword)}>
              {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
            </button>
          </div>

          <input placeholder="Confirmar senha" type={showPassword ? "text" : "password"} value={form.confirmPassword} onChange={(event) => setForm({ ...form, confirmPassword: event.target.value })} />
